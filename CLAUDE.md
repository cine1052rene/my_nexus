...
## 🏪 Play Store 상용 앱 기준 (2026-08-20 확정) ⭐ IMPORTANT

**이 앱은 Google Play Store에 출시할 다중 사용자 상용 앱이다.**
"내 폰에서만 쓰는 개인 앱"이 아니다. 모든 제안·설계·코드 리뷰는 이 전제로 한다.

### 항상 이 관점으로 검토할 것
- **데이터 격리** — 모든 사용자 데이터는 `users/{uid}/` 하위에 둔다. 전역 컬렉션 금지.
  쿼리에서 소유자 필터를 빠뜨려 남의 데이터가 새는 구조를 애초에 만들지 않는다.
- **보안 규칙** — `request.auth != null`만으로 끝내지 말 것. 반드시 `request.auth.uid == userId` 소유권 검사.
- **계정 전환/로그아웃** — 캐시·스트림·프로바이더에 이전 사용자 데이터가 남지 않는지 항상 확인.
  StreamProvider는 `currentUserProvider`를 watch해야 재구독된다.
- **계정 삭제** — Play 정책상 사용자 데이터 **전부** 삭제 가능해야 한다 (`recursiveDelete`).
- **비용** — 기능 하나 추가할 때 "유저 1만 명이면 월 얼마인가"를 먼저 계산한다.
- **개인정보** — Data Safety 신고 대상인지, 개인정보처리방침에 반영해야 하는지 확인.
- **확장성** — 링크 300개 전부 클라이언트로 로드하는 식의 설계는 유저 수가 늘면 깨진다.

### 새 기능 추가 시 체크리스트
1. 이 데이터는 사용자별로 격리되는가?
2. 로그아웃/계정전환 시 잔류하지 않는가?
3. 계정 삭제 시 함께 지워지는가?
4. 유저 1만 명 기준 비용이 감당 가능한가?
5. 새 권한·스코프가 필요하면 Play 심사 대상인가?
