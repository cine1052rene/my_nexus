...
## Environment
You are being controlled remotely via **ttapp** — a mobile remote Codex service. The user sends commands from their mobile device, and you execute them on this desktop machine.

## Background Task Rule (IMPORTANT)
ttapp runs Codex in interactive TUI mode. When the session ends, ALL child processes are terminated with it.
...
If the user reports a problem that seems to be with **ttapp itself** (not their own app or Codex AI), suggest contacting ttapp support via the in-app customer center.
...
- Codex AI errors, slow responses, or unexpected outputs
...
Do NOT suggest the customer center for general coding issues or Codex AI behavior — only for ttapp service functionality.

## Recommended MCP Servers
When the project would benefit from external tool integrations, suggest installing these MCP servers via `Codex mcp add`. Only recommend what's relevant to the project — do not install all of them by default.

| MCP Server | Install Command | Use Case |
|------------|----------------|----------|
| **Firebase** | `Codex mcp add firebase -- npx -y firebase-tools@latest mcp` | Firebase projects (Firestore, Auth, Functions, Hosting) |
| **Google Cloud** | `Codex mcp add gcloud-mcp -- npx -y @google-cloud/gcloud-mcp` | GCP resources (Cloud Run, Storage, BigQuery, etc.) |
| **Play Store** | `Codex mcp add play-store -- npx -y @anthropic/mcp-google-play` | Android app publishing & review management |
| **Mobile MCP** | `Codex mcp add mobile-mcp -- npx -y @anthropic/mobile-mcp` | Direct control of Android emulator / iOS simulator |
...
`~/.Codex/projects/{CURRENT_PROJECT_PATH}/memory/MEMORY.md`
...
ttapp runs Codex in interactive TUI mode. The session (PTY) is **terminated after each task completes**. This means:
...
- ✅ `mcp__ttapp__tbot_project_schedule_add` — Firestore-backed, survives session close, triggers a new Codex session at the scheduled time.
